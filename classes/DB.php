<?php
	...
	abstract class DB
	{
		...
		public static function getAllActores()
		{
		  global $connection;

		  $stmt = $connection->prepare("
		    SELECT a.id AS 'actor_id', a.nombre, a.apellido, a.edad
		    FROM actores as a
		    ORDER BY a.apellido;
		  ");
		  ...
		  foreach ($actores as $actor) {
		    $finalActor = new Actor($actor['id'], $actor['nombre'], $actor['apellido']);

		    $finalActor->setEdad($actor['edad']);
		...
		public static function saveActor(Actor $actor)
		{
		  ...
		  try {
		    $stmt = $connection->prepare("
		      INSERT INTO actores (actor_id, nombre, apellido, edad)
		      VALUES(:actor_id, :nombre, :apellido, :edad)
		    ");

		    $stmt->bindValue(':actor_id', $actor->getId());
		    $stmt->bindValue(':nombre', $actor->getNombre());
		    $stmt->bindValue(':apellido', $actor->getApellido());
		    $stmt->bindValue(':edad', $movie->getEdad());
